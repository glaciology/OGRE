...
//  gnss.enableDebugging();               // Sparkfun_GNSS library debugging messages on Serial
  ...
  initSetup = false;
  //////////////////////////////////////////
  uint8_t currentUART1Setting_ubx = gnss.getVal8(UBLOX_CFG_UART2INPROT_UBX);
...
//  gnss.enableNMEAMessage(UBX_NMEA_VTG, COM_PORT_UART1);
//  gnss.setUART1Output(COM_TYPE_UBX | COM_TYPE_NMEA); // Configure UART1 port to output UBX, NMEA, RTCM3, SPARTN or a combination thereof
  ...
  gnss.setVal8(UBLOX_CFG_MSGOUT_UBX_RXM_RAWX_UART1, 1);
  gnss.setUART1Output(COM_TYPE_UBX); // Configure UART1 port to output UBX, NMEA, RTCM3, SPARTN or a combination thereof

  uint8_t currentUART1Setting_ubx3 = gnss.getVal8(UBLOX_CFG_UART1INPROT_UBX);
  Serial.print("currentUART1Setting_ubx: ");
  Serial.println(currentUART1Setting_ubx3);
  uint32_t currentUART1Baud = gnss.getVal32(UBLOX_CFG_UART1_BAUDRATE);
  Serial.print("currentUART2Baud: ");
  Serial.println(currentUART1Baud);
  uint8_t currentUART1Setting_nmea = gnss.getVal8(UBLOX_CFG_UART1INPROT_NMEA);
  ...
  while(!alarmFlag) { 
    ...
    while (gnss.fileBufferAvailable() >= sdWriteSize) {              // Check to see if we have at least sdWriteSize waiting in the buffer
      ...
      if (Serial.availableForWrite() > 128) {
        ...
        gnss.extractFileBufferData((uint8_t *)&myBuffer, 72); // Extract exactly sdWriteSize bytes from the UBX file buffer and put them into myBuffer
        
        if (!Serial.write(myBuffer, 72)) {                    // Write exactly sdWriteSize bytes from myBuffer to the ubxDataFile on the SD card
